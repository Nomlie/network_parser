...
bash "${SCRIPT_DIR}/00_preflight.sh" \
  Lineage_Supergroup Lineage_clean Resistance_Profile_Collapsed
cd "${PROJECT_ROOT}"
...
RUN_NAME="Hierarchy_with_Supergroup_Lineage_Resistance_02"
...
if [[ "${RUN_LEAKAGE_AWARE_CV}" == "1" ]]; then
  ...
  for label in Lineage_Supergroup Lineage_clean Resistance_Profile_Collapsed; do
