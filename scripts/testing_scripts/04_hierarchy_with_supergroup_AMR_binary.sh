...
bash "${SCRIPT_DIR}/00_preflight.sh" \
  Lineage_Supergroup Lineage_clean AMR_binary
cd "${PROJECT_ROOT}"
...
RUN_NAME="Hierarchy_with_Supergroup_Lineage_AMR_02"
...
if [[ "${RUN_LEAKAGE_AWARE_CV}" == "1" ]]; then
  ...
  for label in Lineage_Supergroup Lineage_clean AMR_binary; do
