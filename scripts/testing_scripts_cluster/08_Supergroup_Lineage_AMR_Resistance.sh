...
bash "${SCRIPT_DIR}/00_preflight.sh" \
  Lineage_Supergroup Lineage_clean AMR_binary Resistance_Profile
cd "${PROJECT_ROOT}"
...
RUN_NAME="Hierarchy_with_Supergroup_Lineage_AMR_Resistance_02"
...
"${PYTHON_BIN}" -m network_parser.cli train-hierarchy \
  ...
  --meta "${META}" \
  --hierarchy_labels Lineage_Supergroup Lineage_clean AMR_binary Resistance_Profile \
...
if [[ "${RUN_LEAKAGE_AWARE_CV}" == "1" ]]; then
  ...
  for label in Lineage_Supergroup Lineage_clean AMR_binary Resistance_Profile; do
