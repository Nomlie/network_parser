#!/usr/bin/env python3
# network_parser/hierarchy_protocol.py
"""
Hierarchy protocol for NetworkParser
====================================

Purpose
-------
Train and apply hierarchical genomic classifiers with two or more ordered levels:

    Root / Level 1: strain / lineage / group placement
    Parent / intermediate levels: nested placement within the path so far
    Child / terminal levels: phenotype endpoints (e.g. AMR binary or resistance profile)

Classic two-label training (``--level1_label`` / ``--level2_label``) and true
multi-level hierarchy training (``--hierarchy_labels``) share this module.

The protocol keeps the NetworkParser architecture explicit:

    input -> DataLoader/preprocessing -> configurable central feature filtering -> root/parent models
                                              -> branch-specific filtering -> child models
                                              -> optional global / parent-conditioned fallbacks
...
except ImportError:  # pragma: no cover - supports running from source tree
...
def _planned_hierarchy_stages(
    config: NetworkParserConfig, train_global_level2: bool
) -> List[str]:
    """Return ordered stage labels for the hierarchy training progress bar."""
    stages = [
    ...
    stages.append("finalize hierarchy registry")
    return stages
    ...
    summary = {
        ...
        "significant_features": int(
            sum(
                ...
                if bool(result.get("significant", False))
            )
        ),
...
def run_feature_panel_check_after_filter(
    ...
    except FeaturePanelThresholdError as exc:
        # Mutate the existing payload before re-raising so hierarchy node error
        # handlers can persist an explicit unsupported panel decision.
        filter_result["central_filtered_matrix"] = filter_result["filtered_matrix"]
        filter_result["feature_panel_separability"] = exc.summary
        filter_result["retained_features"] = []
        raise
...
def load_artifact_filtered_binary_matrix(
    ...
    """
    Pass through the DataLoader-returned sample × marker matrix.

    Phase 6: do **not** rglob arbitrary ``*_binary.tsv`` files from the output
    tree (stale-artifact risk). DataLoader already returns the canonical binary
    matrix in memory; stages must hand that object forward directly.

    ``artifact_root`` is retained for API compatibility and for locating the
    feature manifest only (see ``find_feature_manifest``).
    """
    _ = Path(artifact_root)  # API compatibility; not used for matrix discovery
    if fallback_matrix is None or fallback_matrix.empty:
        raise ValueError(
            "Fallback matrix is empty; cannot continue without DataLoader matrix."
        )
    logger.info(
        "Using DataLoader-returned matrix for downstream modeling "
        "(no recursive *_binary.tsv discovery) | samples=%d | features=%d",
        int(fallback_matrix.shape[0]),
        int(fallback_matrix.shape[1]),
    )
...
def find_feature_manifest(artifact_root: Path) -> Optional[Path]:
    ...
    for candidate in candidates:
        try:
            header = (
                pd.read_csv(candidate, sep="\t", nrows=0).columns.astype(str).tolist()
            )
        except Exception:
            continue
...
def selected_feature_manifest_integrity(
    ...
    context_cols = [
        col
        for col in ["Context_±40", "Context", "Context_sequence", "context_sequence"]
    ...
    for _, row in selected.iterrows():
        ...
        has_ref = _nonempty(
            row, [col for col in ["Ref_allele", "REF"] if col in selected.columns]
        )
        has_alt = _nonempty(
            row, [col for col in ["Alt_allele", "ALT"] if col in selected.columns]
        )
        has_baseline = _nonempty(
            row,
            [
                col
                for col in ["Baseline_allele", "baseline_allele"]
                if col in selected.columns
            ],
        )
    ...
    if context_missing:
        warnings.append(
            "Some selected features lack context sequence needed for raw-sequence query mapping."
        )
    if allele_incomplete:
        warnings.append(
            "Some selected features lack complete REF/ALT/baseline allele metadata."
        )

    raw_ready = not missing_metadata and not context_missing and not allele_incomplete
    status = (
        "query_ready" if raw_ready else ("partial" if len(selected) > 0 else "empty")
    )
    ...
    return {
        ...
        "features_with_complete_allele_metadata": int(
            len(selected) - len(allele_incomplete)
        ),
...
def write_selected_feature_manifest(
    *,
    ...
    if source_manifest is None or source_manifest.empty:
        pd.DataFrame({"Feature_ID": [str(f) for f in features]}).to_csv(
...
def normalize_sample_id(value: Any) -> str:
    """Use the shared sample-ID contract for hierarchy alignment."""
    return normalize_shared_sample_id(value)
...
def align_two_labels(
    ...
    if common.empty:
        raise ValueError(
            "No samples have both level-1 and level-2 labels after alignment."
        )
    ...
    logger.info(
        "Aligned hierarchy supervision | samples=%d | features=%d | level1_classes=%d | level2_classes=%d",
        int(X_final.shape[0]),
        int(X_final.shape[1]),
...
def build_global_level2_training_labels(
    *,
    ...
    logger.info(
        ...
        requested,
        "global_level2_label",
        int(y_global.shape[0]),
        int(y_global.nunique(dropna=True)),
    )
    return X_global, y_global, summary


def align_prediction_matrix(
    X_new: pd.DataFrame, training_features: List[str]
) -> pd.DataFrame:
    """
    Align query matrix to training features.

    Missing trained features are filled with NaN (non-callable), never ordinary 0.
    """
    X_new = X_new.copy()
    ...
    for feature in training_features:
        if feature not in X_new.columns:
            X_new[feature] = float("nan")

    out = X_new.loc[:, training_features].copy()
...
def run_ml_model(
    ...
    model_name: str = "hierarchy_model",
) -> Dict[str, Any]:
    ...
    return runner.run(
        ...
        else getattr(config, "ml_algorithm", "auto"),
...
def train_fallback_rf_model(
    ...
    It keeps the hierarchy protocol runnable while recording the failure clearly.
    """
    ensure_dir(output_dir)
    try:
    ...
    X, missingness_state, missingness_audit = prepare_for_sklearn(X, policy=policy)
    if X.isna().any().any():
        raise ValueError(
            "Fallback RF requires an explicit imputation strategy; non-callable values remain."
        )
    y = y.loc[X.index]
    ...
    payload = {
        ...
        "missingness_state": missingness_state.to_dict(),
    }
...
def train_model_safely(
    ...
    """
    Train a model for the hierarchy protocol.

    Publication-safe behaviour:
    - by default, fail loudly if the requested ML protocol fails;
    - only use the RF fallback if config.allow_hierarchy_rf_fallback=True.
    """
    log_model_input_panel_decision(
        model_name=model_name,
        X=X,
    ...
    try:
        ...
        diagnostics.update(
            {
                ...
                "rf_fallback_enabled": bool(
                    getattr(
                        config,
                        "allow_hierarchy_rf_fallback",
                        getattr(config, "allow_two_level_rf_fallback", False),
                    )
                ),
            }
        )
        ...
        raise RuntimeError(
            ...
            "the global fallback when available. Set allow_hierarchy_rf_fallback=True "
...
def predict_from_model_payload(model_payload: Any, X: pd.DataFrame) -> List[str]:
    ...
    if (
        ...
        and "label_encoder" in model_payload
    ):
...
def get_model_file(model_summary: Dict[str, Any]) -> Optional[str]:
    artifacts = (
        model_summary.get("artifacts", {}) if isinstance(model_summary, dict) else {}
    )
...
def label_distribution_diagnostics(
    ...
    min_samples_for_binary_split = (
        int(counts.shape[0] * 2) if counts.shape[0] >= 2 else 0
    )
...
def apply_level2_class_support_filter(
    ...
    before_diag = label_distribution_diagnostics(
        y0, requested_cv_splits=requested_cv_splits
    )
    ...
    summary: Dict[str, Any] = {
        ...
        "artifacts": {
            "summary_json": str(
                output_dir / "level2_class_support_filter_summary.json"
            ),
        },
    }
    ...
    after_diag = label_distribution_diagnostics(
        y1, requested_cv_splits=requested_cv_splits
    )
    ...
    summary.update(
        {
            ...
            "class_support_table": {
                str(label): int(count)
                for label, count in y1.value_counts(dropna=True).items()
            },
...
def _comma_set(value: Any) -> set:
    ...
    if value is None:
        return set()
    return {
        str(item).strip().lower() for item in str(value).split(",") if str(item).strip()
    }
...
def _read_level2_binary_mapping(mapping_path: str) -> Dict[str, str]:
    ...
    if not mapping:
        raise ValueError(
            "Level-2 binary label mapping file produced no usable mappings."
        )
    return mapping


def _canonical_binary_level2_labels(
    labels: pd.Series, config: NetworkParserConfig
    ...
    susceptible_values = _comma_set(
        getattr(config, "level2_binary_susceptible_values", "")
    )
    ...
    out = pd.Series(
        canonical, index=labels.index, name="level2_binary_label", dtype="object"
    )
    summary = {
        "canonical_classes": sorted([str(v) for v in out.dropna().unique()]),
        "n_unmapped_or_invalid_labels": int(out.isna().sum()),
        "unmapped_or_invalid_label_counts": {
            str(k): int(v) for k, v in dropped_values.items()
        },
    }
    ...
    *,
    ...
    if label_col:
        if label_col not in meta.columns:
            summary.update(
                {"status": "skipped", "reason": "binary_label_column_not_found"}
            )
            write_json(summary, output_dir / "level2_binary_label_summary.json")
            return None, summary
        raw_binary = normalize_labels(
            meta[label_col], drop_missing=True, lowercase=False
        )
        ...
        summary.update(
            {"status": "skipped", "reason": "no_binary_label_source_configured"}
        )
        write_json(summary, output_dir / "level2_binary_label_summary.json")
        return None, summary
    ...
    if common.empty:
        summary.update(
            {"status": "skipped", "reason": "no_overlap_with_binary_level2_labels"}
        )
        write_json(summary, output_dir / "level2_binary_label_summary.json")
        return None, summary
    ...
    summary.update(
        {
            ...
            else "binary_endpoint_has_fewer_than_two_classes",
...
def _successful_level2_fallback_source(
    ...
    if (
        isinstance(global_profile_payload, dict)
        and global_profile_payload.get("status") == "success"
    ):
        return "global_fallback"
    if (
        isinstance(global_binary_payload, dict)
        and global_binary_payload.get("status") == "success"
    ):
...
def classify_ml_failure(exc: Exception) -> str:
    ...
    if isinstance(exc, FeaturePanelThresholdError):
        return "feature_panel_threshold_not_met"
    messages: List[str] = []
    ...
    if "feature-panel" in msg and "threshold" in msg:
        return "feature_panel_threshold_not_met"
    ...
    *,
    ...
    for group_name, payload in subgroup_payload.items():
        ...
        subgroup_summary[str(group_name)] = {
            ...
            "retained_features": int(len(payload.get("features", [])))
    ...
    global_features = (
        global_level2_payload.get("features", [])
    ...
    return {
        ...
        "retained_features_per_level": {
            ...
            "level2_global_fallback": int(len(global_features))
            if isinstance(global_features, list)
            else 0,
            "level2_global_binary_fallback": int(len(global_binary_features))
            if isinstance(global_binary_features, list)
            else 0,
            "level2_by_level1_group": subgroup_summary,
        },
        "model_performance_metrics": {
            "level1": _extract_model_publication_metrics(level1_model),
            "level2_global_fallback": _extract_model_publication_metrics(
                global_level2_payload.get("model", {})
            )
            if isinstance(global_level2_payload, dict)
            else {"status": "unavailable"},
            "level2_global_binary_fallback": _extract_model_publication_metrics(
                global_binary_level2_payload.get("model", {})
            )
            if isinstance(global_binary_level2_payload, dict)
            else {"status": "unavailable"},
        ...
        "known_marker_overlap": {
            ...
            "reason": "known marker set was not provided to the hierarchy protocol",
        },
    }
...
class HierarchyProtocol:
    """Train and apply hierarchical (2+ level) placement and phenotype protocols."""
    ...
    def train_hierarchy(
        ...
        """
        labels = [
            str(label).strip() for label in hierarchy_labels if str(label).strip()
        ]
        if len(labels) < 2:
            raise ValueError(
                "--hierarchy_labels requires at least two metadata columns."
            )
        ...
        write_resource_profile(out, config=self.config, stage="train")
        ...
        if feature_manifest_path is None:
            ...
            logger.info(
                "Using feature manifest for query annotation: %s",
        ...
        if bool(getattr(self.config, "memory_efficient", False)):
            # Avoid multi-GB dense CSV on small machines; keep a compact binary.
            matrix_path = out / "aligned_hierarchy_matrix.parquet"
            try:
                X.to_parquet(matrix_path)
                logger.info(
                    "memory_efficient: wrote aligned matrix as parquet | %s", matrix_path
                )
            except Exception:
                # Fallback: TSV is still heavy but avoids pandas object CSV bloat somewhat
                matrix_path = out / "aligned_hierarchy_matrix.tsv.gz"
                X.to_csv(matrix_path, sep="\t", compression="gzip")
                logger.info(
                    "memory_efficient: wrote compressed TSV matrix | %s", matrix_path
                )
        else:
        ...
        registry = {
            ...
            "training_matrix": {
                ...
                "feature_manifest_file": str(feature_manifest_path)
        ...
        try:
            from network_parser.hierarchy_artifacts import (
                export_hierarchy_dendrogram,
                write_hierarchy_node_dashboard,
            )
        except ImportError:  # pragma: no cover
            from hierarchy_artifacts import (  # type: ignore
                export_hierarchy_dendrogram,
                write_hierarchy_node_dashboard,
            )
        write_hierarchy_node_dashboard(registry, out)
        export_hierarchy_dendrogram(registry_path, out, stem="hierarchy_dendrogram")
    ...
    def _train_hierarchy_node(
        ...
        *,
        ...
        resume = bool(getattr(self.config, "hierarchy_resume_completed_nodes", False))
        try:
            from network_parser.hierarchy_artifacts import try_load_completed_node
        except ImportError:  # pragma: no cover
            from hierarchy_artifacts import try_load_completed_node  # type: ignore
        cached = try_load_completed_node(node_dir, resume=resume)
        if cached is not None:
            needs_children = level_index + 1 < len(hierarchy_labels)
            has_children = bool(cached.get("children"))
            if (not needs_children) or has_children or str(cached.get("status")) in {
                "skipped",
                "constant",
            }:
                return cached
            logger.info(
                "Resume: node summary exists but children missing; retraining %s",
                node_dir,
            )
        ...
        stage_token = (
            f"hierarchy_level_{level_number}__{safe_hierarchy_token(label_column, 40)}"
        )
        if path:
            stage_token = (
                stage_token
                + "__"
                + safe_hierarchy_token("__".join(item["value"] for item in path), 80)
            )
        ...
        if X_node.empty or raw_diag["n_classes"] == 0:
            node_payload.update(
                {"status": "skipped", "reason": "no_aligned_samples_or_labels"}
            )
            ...
            support_table = pd.DataFrame(
                {
                    ...
                    "meets_twofold_cv_minimum": [
                        int(v) >= 2 for v in counts.astype(int).values
                    ],
                }
            )
        ...
        try:
            ...
            failure_reason = classify_ml_failure(exc)
            matrix_for_diag = (
                X_filtered if isinstance(X_filtered, pd.DataFrame) else X_train
            )
            labels_for_diag = (
                y_train.loc[matrix_for_diag.index]
                if isinstance(matrix_for_diag, pd.DataFrame)
                else y_train
            )
            failure_diag = label_distribution_diagnostics(
                labels_for_diag, requested_cv_splits=5
            )
            failure_diag.update(
                {
            ...
            node_payload.update(
                {
                    ...
                    "filter": filter_result.get("summary", {})
                    if isinstance(filter_result, dict)
                    else {},
                    "feature_panel_separability": filter_result.get(
        ...
        if level_index + 1 < len(hierarchy_labels):
            ...
            parallel_children = should_run_parallel(
                self.config,
                enabled_attr="hierarchy_parallel_child_nodes",
                n_tasks=len(child_tasks),
            )
            if parallel_children:
                budget = resolve_parallel_worker_budget(
                    self.config, n_tasks=len(child_tasks)
                )
                outer_jobs = int(budget["outer_jobs"])
                inner_jobs = int(budget["inner_jobs"])
                logger.info(
                    "Training %d hierarchy child nodes in parallel at level %d | "
                    "outer_jobs=%d | inner_jobs=%d | mem_gb=%s | path=%s",
                    len(child_tasks),
                    int(level_number),
                    outer_jobs,
                    inner_jobs,
                    budget.get("available_memory_gb"),
                    path,
                )

                def _train_child_task(task: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
                    payload = run_with_inner_n_jobs(
                        inner_jobs,
                        self._train_hierarchy_node,
                        X=task["X"],
                        labels_df=task["labels_df"],
                        hierarchy_labels=task["hierarchy_labels"],
                        level_index=task["level_index"],
                        sample_index=task["sample_index"],
                        node_dir=task["node_dir"],
                        feature_manifest_df=task["feature_manifest_df"],
                        algorithm=task["algorithm"],
                        explicit_min_samples=task["explicit_min_samples"],
                        path=task["path"],
                    )
                    return str(task["child_value"]), payload

                child_results = Parallel(n_jobs=outer_jobs, prefer="threads")(
                    delayed(_train_child_task)(task) for task in child_tasks
                )
                for child_value, child_payload in child_results:
                    node_payload["children"][child_value] = child_payload
            else:
                for task in progress_iter(
                    child_tasks,
                    desc=f"Hierarchy children L{level_number}",
                    unit="node",
                    leave=False,
                ):
                    node_payload["children"][str(task["child_value"])] = (
                        self._train_hierarchy_node(
                            X=task["X"],
                            labels_df=task["labels_df"],
                            hierarchy_labels=task["hierarchy_labels"],
            ...
            failure_diag.update(
                {
                    "failure_reason": classify_ml_failure(exc),
                    "error": str(exc),
                    "n_features_at_failure": int(X_filtered.shape[1])
    ...
    def _train_hierarchy_terminal_fallbacks(
        ...
        *,
        ...
        """Train optional fallback models for recursive hierarchy query.

        1. Parent-conditioned models for the terminal label (e.g. profile within
           each lineage), when enabled.
        2. A cohort-wide global model for the terminal label only if that label
           is listed in config.hierarchy_global_fallback_labels.

        These fallbacks do not replace the hierarchy. They are used only when a
        deeper branch is skipped because the local slice is too small or
        ...
        try:
            ...
            from hierarchy_artifacts import resolve_global_fallback_label_columns  # type: ignore

        global_labels = resolve_global_fallback_label_columns(labels, self.config)
        train_terminal_global = target_label in global_labels
        train_parent_conditioned = bool(
            getattr(self.config, "hierarchy_train_parent_conditioned_fallbacks", True)
        )

        payload: Dict[str, Any] = {
            "status": "started",
            "target_label_column": str(target_label),
            "preferred_parent_label_column": str(parent_label)
            if parent_label
            else None,
            "fallback_order": ["by_parent_label", "global"],
            "by_parent_label": {},
            "global": {},
            "global_fallback_labels_requested": list(global_labels),
            "train_parent_conditioned_fallbacks": bool(train_parent_conditioned),
            "train_terminal_global_fallback": bool(train_terminal_global),
        }
        ...
        if target_label not in labels_df.columns:
            payload.update(
                {
                    "status": "skipped",
                    "reason": "target_label_missing_from_aligned_metadata",
                }
            )
        ...
        if (
            train_parent_conditioned
            and parent_label is not None
            and parent_label in labels_df.columns
        ):
            ...
            parent_values = sorted(
                [
                    str(v)
                    for v in labels_df[parent_label]
                    .dropna()
                    .astype(str)
                    .str.strip()
                    .unique()
            ...
            for parent_value in parent_values:
                idx = labels_df.index[
                    labels_df[parent_label].astype(str).str.strip() == parent_value
                ]
                model_dir = (
                    output_dir
                    / "by_parent_label"
                    / safe_hierarchy_token(str(parent_label))
                    / safe_hierarchy_token(parent_value)
                )
            ...
            def _train_parent_fallback_task(
                task: Dict[str, Any]
                ...
                return (
                    str(task["parent_label"]),
                    str(task["parent_value"]),
                    model_payload,
                )
            ...
            if parallel_fallbacks:
                budget = resolve_parallel_worker_budget(
                    self.config, n_tasks=len(fallback_tasks)
                )
                outer_jobs = int(budget["outer_jobs"])
                inner_jobs = int(budget["inner_jobs"])
                logger.info(
                    "Training %d parent-conditioned terminal fallbacks in parallel | "
                    "outer_jobs=%d | inner_jobs=%d | mem_gb=%s",
                    len(fallback_tasks),
                    outer_jobs,
                    inner_jobs,
                    budget.get("available_memory_gb"),
                )
                trained_fallbacks = Parallel(n_jobs=outer_jobs, prefer="threads")(
                    delayed(run_with_inner_n_jobs)(
                        inner_jobs, _train_parent_fallback_task, task
                    )
        ...
        elif not train_parent_conditioned:
            payload["by_parent_label"] = {
                "_policy": {
                    "status": "skipped",
                    "reason": "parent_conditioned_fallbacks_disabled",
                }
            }

        if train_terminal_global:
            ...
            payload["global"] = {
                "status": "skipped",
                "reason": "terminal_not_in_hierarchy_global_fallback_labels",
                "target_label_column": str(target_label),
                "features": [],
                "model_file": None,
                "feature_manifest": None,
            ...
            write_json(
                payload["global"],
                ensure_dir(output_dir / "global") / "fallback_summary.json",
            )

        # Optional extra global models for non-terminal labels listed explicitly
        # (e.g. AMR_binary when terminal is Resistance_Profile_Collapsed).
        payload["global_by_label"] = {}
        for glab in global_labels:
            if glab == target_label:
                continue
            if glab not in labels_df.columns:
                continue
            # Lineage globals are handled by _train_hierarchy_global_lineage_fallback
            if "lineage" in glab.lower() and glab != target_label:
                continue
            gdir = ensure_dir(
                output_dir / "global_by_label" / safe_hierarchy_token(glab)
            )
            payload["global_by_label"][glab] = self._train_one_terminal_fallback_model(
                X=X.copy(),
                y=labels_df[glab],
                output_dir=gdir,
                feature_manifest_df=feature_manifest_df,
                algorithm=algorithm,
                target_label_column=glab,
                fallback_scope=f"global_{glab}",
                conditioning_label_column=None,
                conditioning_value=None,
            )
        ...
        for block in payload.get("by_parent_label", {}).values():
            if isinstance(block, dict):
                for model_payload in block.get("models", {}).values():
                    if (
                        isinstance(model_payload, dict)
                        and model_payload.get("status") == "success"
                    ):
                        n_parent_success += 1

        global_success = (
            isinstance(payload.get("global"), dict)
            and payload["global"].get("status") == "success"
        )
        extra_global_success = sum(
            1
            for mp in payload.get("global_by_label", {}).values()
            if isinstance(mp, dict) and mp.get("status") == "success"
        )
        any_success = n_parent_success > 0 or global_success or extra_global_success > 0
        payload.update(
            {
                "status": "success" if any_success else "skipped",
                "reason": "trained_at_least_one_terminal_fallback"
                if any_success
                else "no_trainable_terminal_fallbacks",
                "n_parent_conditioned_fallbacks": int(n_parent_success),
                "global_fallback_available": bool(global_success),
                "n_extra_global_by_label": int(extra_global_success),
            }
        )
        write_json(payload, output_dir / "terminal_fallbacks_summary.json")
        return payload
    ...
    def _train_hierarchy_global_lineage_fallback(
        ...
        *,
        ...
        global_labels = resolve_global_fallback_label_columns(labels, self.config)
        target_label = resolve_hierarchy_global_lineage_fallback_label(
            hierarchy_labels, self.config
        )
        # Train only if this lineage label is in the explicit global list,
        # or legacy mode already expanded it into global_labels.
        allow = bool(target_label) and (
            target_label in global_labels
            or (
                # legacy: hierarchy_train_global_lineage_fallback alone
                str(getattr(self.config, "hierarchy_global_fallback_labels", "none"))
                .strip()
                .lower()
                in {"legacy", "*", "all", "default"}
                and bool(
                    getattr(self.config, "hierarchy_train_global_lineage_fallback", True)
                )
            )
        )
        if not allow:
            payload = {
                "status": "skipped",
                "reason": "lineage_not_in_hierarchy_global_fallback_labels",
                "target_label_column": target_label,
                "global_fallback_labels_requested": list(global_labels),
                "features": [],
                "model_file": None,
    ...
    def _train_one_level2_group(
        ...
        *,
        ...
        (
            ...
            y2_group_train,
            group_class_support,
        ) = apply_level2_class_support_filter(
            X=X_group,
            y=y2_group,
        ...
        label_diag = label_distribution_diagnostics(
            y2_group_train, requested_cv_splits=5
        )
        ...
        fallback_source = _successful_level2_fallback_source(
            global_level2_payload, global_binary_level2_payload
        )
        skip_reasons: List[str] = []
        if label_diag["n_classes"] < 2:
        ...
        if (
            explicit_group_min_n is not None
            and X_group_train.shape[0] < explicit_group_min_n
        ):
            ...
            skip_table = pd.DataFrame(
                {
                    ...
                    "meets_twofold_cv_minimum": [
                        int(v) >= 2 for v in support_table.values()
                    ],
                }
            )
        ...
        try:
            ...
            group_model = train_model_safely(
                ...
                panel_summary=group_filter.get("feature_panel_separability"),
            )
        except Exception as exc:
            failure_reason = classify_ml_failure(exc)
            matrix_for_diag = (
                X_group_filtered
                if isinstance(X_group_filtered, pd.DataFrame)
                else X_group_train
            )
            labels_for_diag = (
                y2_group_train.loc[matrix_for_diag.index]
                if isinstance(matrix_for_diag, pd.DataFrame)
                else y2_group_train
            )
            failure_diag = label_distribution_diagnostics(
            ...
            group_summary.update(
                {
                    ...
                    "error": str(exc),
                    "training_failure_diagnostics": failure_diag,
                    "filter": group_filter.get("summary", {})
                    if isinstance(group_filter, dict)
                    else {},
                    "feature_panel_separability": group_filter.get(
                        "feature_panel_separability", {}
                    )
                    if isinstance(group_filter, dict)
                    else {},
    ...
    def _summarise_hierarchy_node(self, node: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "n_classes": (
                node.get("training_label_diagnostics")
                or node.get("label_diagnostics")
                or {}
            ).get("n_classes"),
            "retained_features": int(len(node.get("features", [])))
    ...
    def train(
        ...
        log_pipeline_header(
            logger,
            "NetworkParser hierarchy training started",
            central_filter=getattr(
                self.config, "central_feature_filter_method", "auto"
            ),
            feature_panel_check=bool(
        ...
        with PipelineProgress(
            _planned_hierarchy_stages(self.config, train_global_level2),
            title="Hierarchy training",
        ) as pipeline_progress:
            return self._train_hierarchy_body(
    ...
    def _train_hierarchy_body(
        ...
        *,
        ...
        log_flow_step(
            logger,
            step="Hierarchy alignment checkpoint",
        ...
        (
            X_global_level2_base,
            y_level2_global_base,
            global_level2_label_summary,
        ) = build_global_level2_training_labels(
            X=X,
            meta=meta,
            y_level2=y_level2,
            level2_label=level2_label,
        ...
        explicit_group_min_n = (
            int(explicit_group_min_n) if explicit_group_min_n is not None else None
        )
        ...
        aligned_labels_df["global_level2_label"] = (
            y_level2_global_base.reindex(X.index)
            .astype("object")
            .where(lambda v: ~v.isna(), "")
            .values
        )
        aligned_labels_df.to_csv(out / "aligned_two_level_labels.csv", index=False)
        write_json(
            global_level2_label_summary, out / "global_level2_label_summary.json"
        )
        ...
        level1_model = train_model_safely(
            ...
            panel_summary=level1_filter.get("feature_panel_separability"),
        )
        log_stage_complete(
            logger,
            3,
            ...
            status=level1_model.get("status", "complete")
            if isinstance(level1_model, dict)
            else "complete",
        )
        ...
        if train_global_level2:
            ...
            (
                X_level2_global_source,
                y_level2_global_source,
            ...
            global_label_diag = label_distribution_diagnostics(
                y_level2_global_source, requested_cv_splits=5
            )
            ...
            if global_label_diag["n_classes"] < 2:
                global_level2_payload = {
                    ...
                    "target_label_column": global_level2_label_summary.get(
                        "global_level2_label_column"
                    ),
                ...
                write_json(
                    global_level2_payload, global_dir / "global_level2_summary.json"
                )
                ...
                try:
                    ...
                    global_model = train_model_safely(
                        ...
                        panel_summary=global_filter.get("feature_panel_separability"),
                    )
                    global_level2_payload = {
                        "status": "success",
                        "filter": global_filter["summary"],
                        "feature_panel_separability": global_filter.get(
                            "feature_panel_separability", {}
                        ),
                        "model": global_model,
                        "features": list(X_global.columns),
                        ...
                        "target_label_source": global_level2_label_summary.get(
                            "source"
                        ),
                        "global_level2_label_summary": global_level2_label_summary,
                        "level2_class_support_filter": global_class_support,
                    ...
                    global_level2_payload = {
                        ...
                        "error": str(exc),
                        "training_failure_diagnostics": failure_diag,
                        "filter": global_filter.get("summary", {})
        ...
        if getattr(self.config, "level2_train_binary_global_fallback", False):
            ...
            if y_binary_source is None:
                global_binary_level2_payload = {
                    "status": "skipped",
                    "reason": binary_label_summary.get(
                        "reason", "binary_label_unavailable"
                    ),
                ...
                write_json(
                    global_binary_level2_payload,
                    binary_dir / "global_binary_level2_summary.json",
                )
                log_branch_decision(
                    logger,
                ...
                X_binary_source = X.loc[
                    X.index.intersection(y_binary_source.index)
                ].copy()
                y_binary_source = y_binary_source.loc[X_binary_source.index].copy()
                (
                    X_binary_train,
                    y_binary_train,
                    binary_class_support,
                ) = apply_level2_class_support_filter(
                    X=X_binary_source,
                    y=y_binary_source,
                    output_dir=binary_dir,
                ...
                binary_label_diag = label_distribution_diagnostics(
                    y_binary_train, requested_cv_splits=5
                )
                ...
                log_flow_step(
                    ...
                    and binary_label_diag["min_class_count"] >= 2
                    else "not_eligible",
                    artifact=binary_class_support.get("artifacts", {}).get(
                        "summary_json"
                    ),
                )

                if (
                    binary_label_diag["n_classes"] < 2
                    or binary_label_diag["min_class_count"] < 2
                ):
                    reason = (
                    ...
                    try:
                        ...
                        binary_model = train_model_safely(
                            ...
                            panel_summary=binary_filter.get(
                                "feature_panel_separability"
                            ),
                        )
                        ...
                        support_table_path = (
                            binary_dir / "global_binary_class_support_diagnostics.tsv"
                        )
                        ...
                        global_binary_level2_payload = {
                            ...
                            "feature_panel_separability": binary_filter.get(
                                "feature_panel_separability", {}
                            ),
                            "model": binary_model,
                            "features": list(X_binary_filtered.columns),
                            ...
                            "level2_class_support_filter": binary_class_support,
                            "level2_label_diagnostics": binary_label_diag,
                            "class_support_table": binary_class_support.get(
                                "class_support_table", {}
                            ),
                        ...
                        matrix_for_diag = (
                            X_binary_filtered
                            if isinstance(X_binary_filtered, pd.DataFrame)
                            else X_binary_train
                        )
                        ...
                        global_binary_level2_payload = {
                            ...
                            "training_failure_diagnostics": failure_diag,
                            "filter": binary_filter.get("summary", {})
                            if isinstance(binary_filter, dict)
                            else {},
                            "feature_panel_separability": binary_filter.get(
                                "feature_panel_separability", {}
                            )
                        ...
                        logger.debug(
                            "Global binary Level-2 training traceback", exc_info=True
                        )
                        log_stage_complete(
                            logger,
                            "5",
        ...
        if parallel_groups:
            budget = resolve_parallel_worker_budget(
                self.config, n_tasks=len(group_tasks)
            )
            outer_jobs = int(budget["outer_jobs"])
            inner_jobs = int(budget["inner_jobs"])
            logger.info(
                "Training %d Level-2 group models in parallel | outer_jobs=%d | "
                "inner_jobs=%d | mem_gb=%s",
                len(group_tasks),
                outer_jobs,
                inner_jobs,
                budget.get("available_memory_gb"),
            )
            trained_groups = Parallel(n_jobs=outer_jobs, prefer="threads")(
                delayed(run_with_inner_n_jobs)(
                    inner_jobs, self._train_one_level2_group, **task
                )
        ...
        subgroup_payload: Dict[str, Any] = {
            str(group_value): group_summary
            for group_value, group_summary in trained_groups
        }
        ...
        registry = {
            ...
            "level1": {
                ...
                "feature_panel_separability": level1_filter.get(
                    "feature_panel_separability", {}
                ),
                "model": level1_model,
                "features": list(X_level1.columns),
            ...
            "level2": {
                ...
                "global_label_column": global_level2_label_summary.get(
                    "global_level2_label_column"
                ),
        ...
        write_json(registry, registry_path)
        logger.info("Hierarchy training complete: %s", registry_path)
        pipeline_progress.complete_stage("finalize hierarchy registry")
        return registry
    ...
    def predict(
        ...
        if not level1_features or not level1_model_file:
            raise ValueError(
                "Registry is missing the level-1 feature list or model file."
            )
        ...
        rows = []
        for sample_id, predicted_group in zip(
            X_level1.index.astype(str), level1_predictions
        ):
            group_payload = (
                registry["level2"]
                .get("by_level1_group", {})
                .get(str(predicted_group), {})
            )
            ...
            if not model_file or not features:
                ...
                if (
                    not fallback
                    or fallback.get("status") != "success"
                    or not fallback.get("model_file")
                ):
            ...
            if model_file and features:
                X_level2 = align_prediction_matrix(
                    X_new_raw.loc[[sample_id]], list(features)
                )
                level2_payload = load_model_payload(model_file)
                level2_prediction = predict_from_model_payload(
                    level2_payload, X_level2
                )[0]
            else:
            ...
            rows.append(
                {
                    ...
                    "level2_target_label_column": registry.get("level2", {}).get(
                        "global_label_column"
                    )
                    if level2_source == "global_fallback"
                    else registry.get("level2", {}).get("label_column"),
                }
            )
        ...
        logger.info("Hierarchy prediction complete: %s", predictions_path)
        return predictions
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train or apply a hierarchical NetworkParser model (2+ levels): placement first, phenotype endpoints nested under parent branches.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    sub = parser.add_subparsers(dest="command", required=True)

    train = sub.add_parser(
        "train", help="Train the hierarchy protocol (two-level or multi-level)."
    )
    train.add_argument(
        "--genomic", required=True, help="Genomic matrix file or VCF directory."
    )
    ...
    train.add_argument(
        "--global_level2_label",
        default=None,
        help="Optional metadata column for the standard global Level-2 fallback, e.g. AMR_binary.",
    )
    ...
    train.add_argument(
        "--algorithm",
        default=None,
        help="Optional ML algorithm override passed to MLProtocolRunner.",
    )
    train.add_argument(
        "--no_global_level2",
        action="store_true",
    ...
    train.add_argument(
        ...
        action="store_true",
        help="Train an additional resistant/susceptible global Level 2 fallback model across all lineages.",
    )
    train.add_argument(
        "--level2_binary_label_column",
        default=None,
        help="Metadata column containing resistant/susceptible labels for the binary fallback model.",
    )
    train.add_argument(
        "--level2_binary_label_mapping_file",
        default=None,
        help="CSV/TSV mapping file from detailed Level 2 labels to resistant/susceptible labels.",
    )
    train.add_argument(
        "--level2_binary_resistant_values",
        default=None,
        help="Comma-separated values interpreted as resistant for the binary fallback.",
    )
    train.add_argument(
        "--level2_binary_susceptible_values",
        default=None,
        help="Comma-separated values interpreted as susceptible for the binary fallback.",
    )
    train.add_argument(
    ...
    predict = sub.add_parser(
        "predict", help="Apply a trained hierarchy protocol to new strain/sample input."
    )
    ...
    predict.add_argument(
        "--registry",
        required=True,
        help="Path to hierarchy/two-level model registry JSON from training.",
    )
    predict.add_argument(
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if getattr(args, "low_support_review_min_class_count", None) is not None:
        config.low_support_review_min_class_count = int(
            args.low_support_review_min_class_count
        )
    ...
    if getattr(args, "hierarchy_global_lineage_fallback_label", None) is not None:
        config.hierarchy_global_lineage_fallback_label = (
            args.hierarchy_global_lineage_fallback_label
        )
    if bool(
...
TwoLevelProtocol = HierarchyProtocol
